...
Weil die Iteratee IO-Streams von Oleg Kiselyov öffentlich vorgestellt wurden, werden in diesem Abschnitt die Hintergründe dieser Streams nach Kiselyov vorgestellt.
Als Quellen hierfür dienen \citealt{kiselyov2012} und \citealt{iteratee_io}.
...
Iteratee IO-Streams besteht aus vier Komponenten.
...
\lstinline|Iteratee| konsumiert \lstinline|Stream|-Elemente.
\lstinline|Enumerator| generiert \lstinline|Stream|-Elemente.
...
\paragraph{Stream} % (fold)
\label{par:stream}\mbox{}

\lstinline|Stream| hat den Typ
\begin{lstlisting}[language=Haskell]
data Stream el = EOF (Maybe ErrMsg) | Chunk [el]
\end{lstlisting}

Dieser Typ ist parametrisiert über den Typ der gehaltenen Elemente \lstinline|el|.

Für den Fall, dass die Datenquelle nicht erschöpft ist, existiert der Konstruktor \lstinline|Chunk el|.
...
Für den Fall, dass die Datenquelle erschöpft, oder ein Fehler aufgetreten ist, existiert der Konstruktor \lstinline|EOF (Maybe ErrMsg)|.

% paragraph stream (end)
