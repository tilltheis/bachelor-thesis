...
Dadurch ist es möglich, mehrere \lstinline|Enumerator|s auf einen \lstinline|Iteratee| anzuwenden.
Um ein \lstinline|Input.EOF|-Element an ein \lstinline|Iteratee| zu senden, wird die \lstinline|run|-Methode des \lstinline|Iteratee|-Traits aufgerufen.
...
\begin{enumerate}
  \item Durch Erstellung einer neuen Klasee, die von \lstinline|Enumerator| erbt und \lstinline|apply| implementiert.
...
Weil zum Befüllen eines \lstinline|Iteratee|s nur die \lstinline|fold|-Methode zur Verfügung steht, wurde für dieses Beispiel die \lstinline|folder|-Funktion aus dem vorigen Kapitel übernommen.
...
Diese Methode nimmmt als Parameter beliebig viele Elemente eines Typs und wird diese bei Anwendung in \lstinline|Input|-Elemente verpacken und an den \lstinline|Iteratee| übergeben.
...
Das Ergebnis der Anwendung eines \lstinline|Enumerator|s ist der \lstinline|Iteratee| nach Konsum der Elemente in der \lstinline|Future|-Monade.
Ein Beispiel dafür ist in Listing~\ref{lst:iterateeausfuehrung1} zu sehen.

\begin{lstlisting}[caption=Ausführung eines \lstinline|Iteratee|s durch einen \lstinline|Enumerator|, label=lst:iterateeausfuehrung1]
...
Nachdem ein \lstinline|Enumerator| alle Elemente an einen \lstinline|Iteratee| übergeben hat, sendet er allerdings kein \lstinline|Input.EOF|-Element.
Dadurch ist es möglich, mehrere \lstinline|Enumerator|s auf einen \lstinline|Iteratee| anzuwenden, wie in Listing~\ref{lst:iterateeausfuehrung2}.

\begin{lstlisting}[caption=Ausführung eines \lstinline|Iteratee|s durch einen zweiten \lstinline|Enumerator|, label=lst:iterateeausfuehrung2]
...
Um ein \lstinline|Input.EOF|-Element an ein \lstinline|Iteratee| zu senden, wird die \lstinline|run|-Methode des \lstinline|Iteratee|-Traits aufgerufen.
Ein Beispiel dafür ist in Listing~\ref{lst:iterateeausfuehrung3} zu sehen.

\begin{lstlisting}[caption=Ausführung eines \lstinline|Iteratee|s beenden, label=lst:iterateeausfuehrung3]
val sumResult: Future[Future[Future[Int]]] =
  sumIterateeAfterE2.map(_.map(_.run))
\end{lstlisting}

Weil es aber sehr mühsam ist, den \lstinline|Iteratee| innerhalb der \lstinline|Future|-Monade zu bearbeiten, ist es möglich \lstinline|Future[Iteratee[E, A]]| in einen \lstinline|Iteratee[E, A]| zu transformieren.
...
Unter Einsatz dieser Methode wird der Code wesentlich einfacherer und Verständlicher, wie in Listing~\ref{lst:iterateeausfuehrung_mit_flatten} zu sehen ist.

\begin{lstlisting}[caption=Ausführung eines \lstinline|Iteratee|s mit Einsatz von \lstinline|Iteratee.flatten|, label=lst:iterateeausfuehrung_mit_flatten]
