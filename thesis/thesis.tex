...
Das \lstinline|Input.Empty| "`signifies a stream with no immediately available data but which is still continuing"' \cite[vgl.][]{iteratee_io}\todo{Ist Übernehmen von Kiselyovs Definition für Plays Implementierung erlaubt?}.
...
\subsubsection{Iteratee} % (fold)
\label{ssub:design_iteratee}
...
Ein \lstinline|Iteratee|, der mit seiner Berechnung fertig ist, ist im Zustand \lstinline[breaklines=true]|Step.Done|.
...
% subsubsection design_iteratee (end)


\subsubsection{Enumerator} % (fold)
\label{ssub:design_enumerator}
...
% subsubsection design_enumerator (end)
