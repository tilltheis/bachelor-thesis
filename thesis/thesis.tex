...
\paragraph{Enumeratees} % (fold)
\label{par:enumeratees}\mbox{} % force new line
...
\lstinline|Iteratee|s und \lstinline|Enumerator|s können sequentiell oder parallel komponiert werden.
Bei der Komposition werden zwei \lstinline|Iteratee|s oder \lstinline|Enumerator|s zu einem neuen \lstinline|Iteratee| oder \lstinline|Enumerator| kombiniert.
...
Parallele Komposition von \lstinline|Enumerator|s bedeutet, dass mehrere \lstinline|Enumerator|s auf nicht-sequentielle Weise kombiniert werden.
So kann beliebig gewählt werden, aus welchem der zur Verfügung stehenden \lstinline|Enumerator|s gelesen wird.
Dabei ist es durchaus denkbar, dass Elemente mehrerer \lstinline|Enumerator|s zu einem Element kombiniert werden.
Auch ist es möglich, dass in Abhängigkeit der Elemente eines \lstinline|Enumerator|s, ein anderer \lstinline|Enumerator| für das nächste Element ausgesucht wird.
Beispielsweise kann abwechselnd ein Name aus der einen Datei und dazugehörige Informationen aus einer anderen Datei gelesen und anschließend zusammengefasst werden.

Es können auch Kompositionen mit anderen Abhängigkeiten zwischen zwei \lstinline|Iteratee|s oder \lstinline|Enumerator|s gebildet werden.
Sequenzielle und parallele Komposition sind allerdings zwei typische Varianten.
