...
Diese Komponenten und ihr jeweiliger Verwendungszweck wird in den folgenden Abschnitten vorgestellt.
...
\subsubsection{Parallele Komposition} % (fold)
...
Im folgenden werden mehrmals folgende Typ-Aliase und eine Funktion verwendet:

\begin{lstlisting}[caption=Verwendete Typ-Aliase, label=lst:verwendete_typ_aliase]
...
Die in den Code-Ausschnitten gezeigen Variablen \lstinline|left| und \lstinline|right| haben immer die gleiche Bedeutung.
...
val right: I[String] =
  alternative(i.flatMap(k1), i.flatMap(k2))
    ...
    \lstinline|alternative| komponiert die Ergebnis-\lstinline|Iteratee|s der beiden Funktionen \lstinline|k1| und \lstinline|k1|.
    Der resultierende \lstinline|Iteratee| erkennt, was der am schnellsten erkennende Eingabe-\lstinline|Iteratee| erkennt.
    Idempotenz wird von \citeauthor{kiselyov2012}, wie in Listing~\ref{lst:idempotenz_eines_iteratees} zu sehen, definiert.
