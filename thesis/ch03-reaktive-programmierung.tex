...
$T$ ist eine Abbildung von einem Objekt der Kategorie $C$ auf ein anderes und dient als Typ des Tripels.\todo{Korrekt? Eigene Interpretation. Quellen nicht auffindbar.}
...
Die ersten Generatoren innerhalb einer \lstinline|for|-Comprehension werden zu \lstinline|flatMap|s und der letzte Generator wird zu \lstinline|map| \cite[vgl.][S.~525]{programming_in_scala}\todo{Seiten in der Print-Version nachschlagen}.
...
Ein \lstinline|Future| ist ein Platzhalter für das Ergebnis einer nebenläufigen Berechnung und hält nach Beendigung dieser Berechnung dessen Ergebnis.
Ein \lstinline|Future| ist an ein \lstinline|Promise| gekoppelt, welches einmalig den Wert des \lstinline|Future|s setzen kann.
...
Ein \lstinline|Future| ein \gls{adt}, der entweder für ein erfolgreich berechnetes Ergebnis, oder für einen aufgetretenen Fehler steht.
...
Dieser Abschnitt beschränkt sich allerdings auf die monadischen Operationen auf \lstinline|Futures|.
...
Die nebenläufigen Berechnungen, die von \lstinline|Future|s ausgeführt werden, werden innerhalb von \lstinline|ExecutionContext|s ausgeführt.
Ein \lstinline|ExecutionContext| ist vergleichbar mit einem Thread-Pool\todo{braucht das eine Definition?} und ist dafür zuständig, dass an ihn delegierte Aufgaben im Hintergrund ausgeführt werden.
...
Unter anderem beinhaltet dieses Objekt die Methode \lstinline|unicast|, die imperatives schreiben auf einen \lstinline|Iteratee| erlaubt.\todo{durch `broadcast` ersetzten}
...
val right: I[String] = failure
  ...
  \item Rechtsdistributivität
    ...
    Eine Funktion $f$ ist idempotent, wenn $f(x) = f(f(x))$\todo{Quelle}, mehr dazu im nächsten Absatz.
