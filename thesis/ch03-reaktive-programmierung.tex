...
Monaden sind ein Konzept zur Strukturierung von Programmen, das ursprünglich aus der Mathematik, genauer der Kategorientheorie stammt.
\citealt{monads_program_structure} nutzte Monaden für Beweise im Kontext des Lambda-Kalküls, worauf \citealt{monads_programming} aufbaute, um Monaden im Umfeld einer Programmiersprache zu verwenden.
...
Eine Monade in einer Kategorie $C$ \todo{Kategorie erklären} ist ein Tripel $T = (T, \eta, \mu)$, wobei $T$ ein Funktor $T: C \to C$ ist und $\eta$ und $\mu$ natürliche Transformationen mit $\eta: Id_C \to T$ und $\mu: T^2 \to T$ sind \citetext{vgl.~\citealp[S.~137]{monads_category_theory}, \citealp[S.~2]{monads_program_structure}}.
Ein Funktor $T: C \to B$ ist ein Morphismus \todo{Morphismus erklären} von einer Kategorie $C$ nach einer Kategorie $B$ \cite[vgl.][S.~13]{monads_category_theory}.
\todo{Kleisli-Tripel}
...
Eine Monade ist eine Art Container mit den drei assoziierten Operationen.
...
\todo{bind(f,m) nach bind(m,f) ändern (map analog)}
Ein Kleisli-Tripel über einer Kategorie $C$ ist $(T, \eta, \_^*)$, wobei $T: Obj(C) \to Obj(C)$, $\eta_A: A \to T A$ und $^*$ ein Operator ist, der Morphismen der Art $f: A \to T B$ nach $f^*: T A \to T B$ umformt \cite[vgl.][S.~2]{monads_program_structure}.
...
In Haskell \cite[vgl.][S.~2]{monads_haskell}, als auch in Scala \cite[vgl.][]{monads_scala} werden sie durch Kleisli-Tripel umgesetzt.

Scala hat Monaden via \lstinline|for|-Comprehensions fest in die Sprache integriert und nutzt dafür Operationen des Kleisli-Tripels.
...
Die \lstinline|map|-Methode entspricht hierbei der \lstinline|map|-Funktion und die \lstinline|flatMap|-Methode entspricht der \lstinline|bind|-Funktion aus dem vorigen Unterabschnitt.
...
Auch wenn Scalas for-Comprehensions Monaden nach dem Kleisli-Tripel erwarten, so haben die Implementationen der Monaden in der Standardbibliothek i.~d.~R. auch die Methode \lstinline|flatten|, die der als \lstinline|join| vorgestellten Funktion entspricht.
Bevor weiter auf for-Comprehensions eingegangen wird, sollen einige Beispiele gezeigt werden, um das Konzept zu verdeutlichen.
...
Optionale Daten sind Daten, die vielleicht nicht vorhanden sind und würden in Sprachen, wie z.~B. Java als \lstinline|null| dargestellt werden \cite[vgl.][S.~329]{programming_in_scala}\todo{Seite verifizieren}.\todo{Auf algebraischen Datentyp eingehen?}
...
Wenn im Kontext der Altersstatistiken-Anwendung zwei Altersgruppen zusammengezählt werden sollen, könnte dies aussehen, wie in Listing~\ref{lst:die_option_monade}.\todo{models.AgeStatistics aktualisieren}
...
Anschließend werden beide Ergebnisse mit Hilfe der Monaden-Operationen kombiniert.\todo{AgeStatistics ohne Default-Value, um Beispiel sinnvoller zu machen}
...
\subsection{Die List-Monade} % (fold)
...
% subsection die_list_monade (end)
...
for-Comprehensions bieten eine alternative Schreibweise für die zuvor vorgestellten Monaden-Operationen.
Durch die Notation mit for-Comprehensions lassen sich einige längere Ausdrücke, die diese Operationen verwenden, verständlicher ausdrücken.
for-Comprehensions haben die in Listing~\ref{lst:monaden_operationen_mit_for_comprehension} gezeigte Form.
...
Die ersten Generatoren innerhalb einer for-Comprehension werden zu \lstinline|flatMap|s und der letzte Generator wird zu \lstinline|map| \cite[vgl.][S.~525]{programming_in_scala}\todo{Seiten in der Print-Version nachschlagen}.
