...
Für diese Statistik sollen nur der Alterszahlen und Personenzahl des jeweiligen Alters gesammelt werden.
...
Um die Verwendung noch komfortabler zu machen, wird in \lstinline|app/models/AgeStatistics.scala| ein Objekt mit weiteren Konstruktormethoden erstellt, wie in Listing~\ref{lst:das_agestatistics_hilfsobjekt} zu sehen.
Dadurch, dass \lstinline|AgeStatistics.empty| für undefinierte Schlüssel einen Standardwert von null zurückgibt, können auch unbekannte Alterseinträge abgefragt werden, ohne dass ein Fehler auftritt.
...
object Application extends Controller {
  val ageStatistics = AgeStatistics.sample
  def index = Action {
...
Das \lstinline|@|-Symbol führt einen Scala-Ausdruck an, der in geschweifte Klammern eingeschlossen werden kann \cite[vgl.][]{play_templates}.
...
Das Zeichnen des Diagramms erfolgt client-seitig via \gls{js} und ist in \lstinline|public/javascripts/main.js| ausgelagert.
...
\lstinline|makeAgeStatisticsChart| erwartet neben dem \gls{css}-Selector des Diagramm-Elements die Altersstatistiken, die auf Server-Seite als \lstinline|Map| vorliegen.
Diese werden auf der \gls{js}-Seite als Objekt erwartet, weil \gls{js} keine \lstinline|Map|s kennt, aber Objekte eine ähnliche Funktionalität bieten.
Weil \gls{js}-Objekte die Schlüsselwerte als \lstinline|String| erwarten, müssen die Schlüssel serverseitig erst in \lstinline|String|s konvertiert werden.
Anschließend kann mittels \lstinline|Json.toJson| \todo{dazu später mehr} die \lstinline|Map| in ein \gls{js}-Objekt konvertiert und mit \lstinline|toString| und \lstinline|Html| in das Template eingefügt werden.
